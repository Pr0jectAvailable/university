...
int menu(void)
{
	...
	cout << "6. Загрузка с файла" << endl;
	cout << "7. Добавление L в начало списка" << endl;
	cout << "8. Пустой ли список" << endl;
	cout << "9. Выход" << endl;
	cout << endl;
	do
	{
...
bool addLBegin(Address** phead, Address** plast, Address** pstart_list, Address** plast_list){
	Address* head = *pstart_list;
	Address* tail = *plast_list;
	Address* old_head = *phead;
	...
	while (true)
	{
		...
		case 6: {
			...
			readFromFile(&head, &last);
			  ...
			  break;
		case 7: {
			Address* end = new Address{"Gaya", "Minsk", nullptr, nullptr};
			Address* start = new Address{ "Taiga", "Hz_gde", end, nullptr };
			end->prev = start;

			cout << "Добавление L \n";
			addLBegin(&head, &last, &start, &end);
		}
			  break;
		case 8: {
			cout << "Пустой:" << (returnN(head) ? "да" : "нет") << endl;
		}
			  break;
		case 9:  exit(0);
		default: exit(1);
