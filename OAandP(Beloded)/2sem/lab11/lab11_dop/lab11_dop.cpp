...
int getSize(Number* begin);
void deleteFirstNegative(Number** begin, Number** end, int* count);
...
int main() {
    ...
    do {
        ...
        cout << "5 удалить первый отрицательный элемент" << endl;
        cout << "6 определить размер очереди" << endl;
        cout << "0 выход" << endl;
        cout << "Ввод: ";
        cin >> choice;
        ...
        case 4:
            ...
            break;
        case 5:
            deleteFirstNegative(&begin, &end, &count);
            break;
        case 6:
            cout << "размер: " << getSize(begin) << endl;
            break;
        case 0:
            cout << "выход" << endl;
            break;
...
void deleteFirstNegative(Number** begin, Number** end, int* count) {
    ...
    while (t != NULL && t->info >= 0) {
        prev = t;
        t = t->next;
    }

    if (t == NULL) {
        cout << "нет отрицательныъ" << endl;
        return;
    }
    ...
    delete t;
    (*count)--;
    cout << "первый отрицательный удалён" << endl;
}
...
//8	Создать очередь для целых чисел и функции для ввода, вывода,
//удаления и определения размера очереди.Разработать функцию,
//которая удаляет из очереди первый отрицательный элемент, если такой есть.